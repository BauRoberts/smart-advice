...
export function searchCnaeCodes(query: string): CnaeOption[] {
  ...
  return cnaeOptions.filter(option => 
...
export function getCnaeByCode(code: string): CnaeOption | undefined {
  return cnaeOptions.find(option => option.code === code);
}
...
export function determineEmpresaTipo(cnaeCode: string): 'manufactura' | 'servicios' {
  // CNAE codes starting with 'C' (10-33) are manufacturing
  if (cnaeCode.startsWith('C') || 
      (cnaeCode.length >= 2 && parseInt(cnaeCode.substring(0, 2)) >= 10 && 
       parseInt(cnaeCode.substring(0, 2)) <= 33)) {
    return 'manufactura';
  }
  
  // All others are considered services
  return 'servicios';
}